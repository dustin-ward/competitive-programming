...
double cp(Point P[],int n,vector<Point>& X,int l,int h){
  ...
  M.x += d, M.y = 1e100; int t=stable_partition(P+m,P+h,left_half)-P;
...
double outerAngle(Point a, Point b, Point c) {
//	return PI - acos(
...
int main() {
	int N,D,G; cin>>N>>D>>G;
	Point polygon[61], hull[61];
	for(int i=0; i<N; ++i)
		cin>>polygon[i].x>>polygon[i].y;

	N = convex_hull(polygon, N, hull);
	double ans = area_polygon(hull, N);
	
	if(G > 0) {
		for(int i=0; i<N; ++i) {
			Point a = hull[i], b = hull[(i+1)%N], c = hull[(i+2)%N];
			ans += D * G * len(b-a);
			ans += fracCircArea(outerAngle(a,b,c), D*G);	
		}
	}
