...
int main() {
    ...
    for(int i=0; i<N; ++i) {
        ...
        for(int j=0; j<k; ++j) {
            int toy; cin>>toy;
            // cout<<toyIdx+toy-1<<" -> "<<kidIdx+i<<" (1)"<<endl;
            G.add_edge(toyIdx+toy-1, kidIdx+i, 1);
        }
        // cout<<kidIdx+i<<" -> "<<snk<<" (1)"<<endl;
        G.add_edge(kidIdx+i, snk, 1);
	}
    ...
    for(int i=0; i<P; ++i) {
        ...
        for(int j=0; j<l; ++j) {
            int toy; cin>>toy;
            // cout<<groupIdx+i<<" -> "<<toyIdx+toy-1<<" (1)"<<endl;
            G.add_edge(groupIdx+i, toyIdx+toy-1, 1);
            S.insert(toy-1);
        }
        int cap; cin>>cap;
        // cout<<src<<" -> "<<groupIdx+i<<" ("<<cap<<")"<<endl;
        G.add_edge(src, groupIdx+i, cap);
    }

    for(int i=0; i<M; ++i) {
        if(S.count(i)) continue;
        G.add_edge(src, toyIdx+i, 1);
    }

    int f = network_flow(G, src, snk);
    cout<<f<<endl;
}
