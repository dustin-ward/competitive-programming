...
int main() {
    ...
    for(int i=0; i<M; i++) {
        for(int j=0; j<N; j++) {
            if(B[i][j] == 'B') {
                G.add_edge(src,idx(i,j,IN),INF);
                G.add_edge(idx(i,j,IN),idx(i,j,OUT),INF);
            }
            else if(B[i][j] == '.')
                G.add_edge(idx(i,j,IN),idx(i,j,OUT),INF);
            else
                G.add_edge(idx(i,j,IN),idx(i,j,OUT),C[B[i][j]-'a']);

            if(i==0 || i==M-1 || j==0 || j==N-1)
                G.add_edge(idx(i,j,OUT),snk,INF);

            for(int k=0; k<4; k++) {
                int i2 = i+dy[k];
                int j2 = j+dx[k];
                if(bounds(i2,j2))
                    G.add_edge(idx(i,j,OUT),idx(i2,j2,IN),INF);
            }
        }
    }
