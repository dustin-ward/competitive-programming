...
int idx(int r, int c, int h, int dir){
...
int main() {
    ...
    for(int i=0; i<n; i++) {
        for(int j=0; j<n; j++) {
            cin>>field[i][j];
            for(int k=0; k<=h; k++)
                G.add_edge(idx(i,j,k,0), idx(i,j,k,1), 1);
        }
    }
    
    for(int i=0; i<k; i++) {
        int r,c; cin>>r>>c;
        G.add_edge(src, idx(r,c,0,0), 1);
    }
    ...
    for(int i=0; i<h; i++) {
        ...
        for(int j=0; j<n; j++) {
            for(int k=0; k<n; k++) {
                if(field[j][k] <= l) continue;
                for(int m=0; m<5; m++) {
                    int r=j+dy[m], c=k+dx[m];
                    if(r >= 0 && r < n && c >= 0 && c < n)
                        G.add_edge(idx(r,c,i,1), idx(j,k,i+1,0), 1);
                }
                if(i == h-1)
                    G.add_edge(idx(j,k,i+1,1), snk, 1);
            }
        }
    }
