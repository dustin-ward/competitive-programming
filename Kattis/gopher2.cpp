...
int main() {
    ...
    while(cin>>n>>m>>s>>v) {
        Graph g(n+m+2);
        vector<pair<double,double>> gophers(n, make_pair(0,0)), holes(m, make_pair(0,0));
        int src = 0;
        int gIdx = 1;
        int hIdx = n+1;
        int snk = n+m+1;
        for(int i=0; i<n; i++) {
            ...
            g.add_edge(src, gIdx+i, 1);
        }
        for(int i=0; i<m; i++) {
            cin>>holes[i].first>>holes[i].second;
            g.add_edge(hIdx+i, snk, 1);
        }

        double safeDistance = v*s;
        for(int i=0; i<n; i++) {
            for(int j=0; j<m; j++) {
                double dx = fabs(gophers[i].first - holes[j].first);
                double dy = fabs(gophers[i].second - holes[j].second);
                if((dx*dx + dy*dy) <= safeDistance*safeDistance)
                    g.add_edge(gIdx+i, hIdx+j, 1);
            }
        }

        cout<<n - network_flow(g, src, snk)<<endl;
    }
}